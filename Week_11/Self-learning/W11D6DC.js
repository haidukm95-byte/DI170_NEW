//1. Use this Giphy API Random documentation. Use this API Key : hpvZycW22qCjn5cRM1xtWB8NKq4dQ2My
//2. In the HTML file, add a form, containing an input and a button. This input is used to fetch gif 
//depending on a specific category.
//3. In the JS file, create a program to fetch one random gif depending on the search of the user 
//(ie. If the search is “sun”, append on the page one gif with a category of “sun”).
//4. The gif should be appended with a DELETE button next to it. Hint : to find the URL of the gif, 
//look for the sub-object named “images” inside the data you receive from the API.
//5. Allow the user to delete a specific gif by clicking the DELETE button.
//6. Allow the user to remove all of the GIFs by clicking a DELETE ALL button.

//1. Within div id='main', create div class='gif'
//2. Add event listener to ADD button according to query value:
// - fetch a random gif according to the query
// - append a gif to div class='gif'
// - append a DELETE button to div class 'gif'
//<!--1.2 Create REMOVE ALL button and place it next to ADD button-->
//<!--1.3 optionally - create an event for REMOVE ALL button when it asks 'are you sure? Y/N'-->

const gifAdd=document.querySelector('#gifAdd');
const removeAll= document.querySelector('#removeAll');
const main=document.querySelector('#main');
const input=document.querySelector('#input');
//`api.giphy.com/v1/gifs/random?tag=${input.value}&rating=g&api_key=hpvZycW22qCjn5cRM1xtWB8NKq4dQ2My`

gifAdd.addEventListener('click', async ()=>{
    const query=input.value.trim();
    if(!query) return;

    const apiKey='hpvZycW22qCjn5cRM1xtWB8NKq4dQ2My';
    const url = `https://api.giphy.com/v1/gifs/random` + `?tag=${encodeURIComponent(query)}&rating=g&api_key=${apiKey}`;
        try {
            const resp = await fetch(url);
            //fetch() returns a promise that resolves to a Response object when the
//HTTP request completes. Because the enclosing callback is async, we can
//await that promise and store the result in resp.
            if (!resp.ok) throw new Error(resp.statusText);
//resp.ok is a convenience Boolean that’s true for 2xx status codes. If the
//server returned 404, 500, etc., we consider that a failure and throw an error;
//statusText contains the textual status message sent by the server
//(“Not Found”, “Internal Server Error”, …).
            const json = await resp.json();
//The Response object has helper methods for reading the body; json() reads
//the body stream and parses it as JSON, returning another promise. Again we
//await it and end up with a plain JavaScript object in json.
            const imageUrl = json.data?.images?.original?.url;
//The structure of the data returned by Giphy is nested: data.images.original.url
//is where the actual GIF URL lives. The ?. operators are optional‑chaining
– //if any of the intermediate properties are missing (data is undefined,
//images doesn’t exist, …) then the whole expression evaluates to undefined
//instead of throwing. This guards against malformed responses.
            if (!imageUrl) throw new Error('no gif in response');
//Finally, if we didn’t manage to extract a URL (because the API returned
//something unexpected, or the search term yielded no result), we throw an error
//so the catch block above can handle it. If everything is fine, imageUrl
//now holds a string you can assign to an <img> element’s src and display
//the GIF.

            const container = document.createElement('div');
            container.className = 'gif';

            const img = document.createElement('img');
            img.src = imageUrl;
            container.appendChild(img);

            const del = document.createElement('button');
            del.className = 'delete';
            del.textContent = 'DELETE';
            del.addEventListener('click', () => container.remove());
            container.appendChild(del);

            main.appendChild(container);
        } catch (err) {
            console.error(err);
        }
});

removeAll.addEventListener('click', () => {
    while (main.firstChild) main.removeChild(main.firstChild);
        if (!confirm('Are you sure?')) return;
});


//A few extra tips:

//always await or use proper .then() callbacks on fetch before
//inspecting the returned data.
//create DOM elements (document.createElement('img')) rather than trying to
//append strings.
//don’t call console.error() inside .catch(); give .catch a function.
//put the “delete all” handler on a separate button and clear #main in one go.