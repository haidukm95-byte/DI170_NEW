...
SELECT CONCAT(first_name, ' ', last_name) AS alias_name
FROM customer;
...
SELECT title, description, replacement_cost FROM film ORDER BY replacement_cost ASC FETCH FIRST 10 ROWS ONLY;
--11. Not satisfied with the results. Write a query which will find the next 10 cheapest movies.
   -- Bonus: Try to not use LIMIT.
SELECT title, description, replacement_cost FROM film ORDER BY replacement_cost ASC, title ASC OFFSET 10 ROWS FETCH NEXT 10 ROWS ONLY;
...
WHERE inventory.inventory_id=Null;
...
customer.last_name, payment.customer_id, payment.amount, payment.payment_date,
payment.staff_id, staff.staff_id, staff.first_name, 
staff.last_name
FROM customer
INNER JOIN payment
