...
export default function InventoryGeneral() {
    useEffect(() => {
            document.title = 'General Inventory - Warehouse App';
        }, []);
    ...
    async function runSearch(e) {
        ...
        try {
            ...
            const { data } = await api.get('/manager/inventory/general', { params });
            setItems(data.items);
            setLoaded(true);
        } catch (err) {
            setError(err.response?.data?.error ?? 'Failed to load general inventory');
        }
    }
