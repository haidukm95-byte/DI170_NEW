//Instructions

//In todays exercise we will be creating a Mad Libs game.
//If you’ve never played this game, a mad lib is a game where you fill in a 
//  bunch of blank inputs with different word types (ie : noun, adjective, 
//  verb), and then a story is generated based on the words you chose, and 
//  sometimes the story is surprisingly funny.

//In this exercise you work with the HTML code presented below.

//Follow these steps :

//    Get the value of each of the inputs in the HTML file when the form 
// is submitted. Remember the event.preventDefault()

const libButton=document.querySelector('#lib-button');
const storySpan=document.querySelector('#story');
libButton.addEventListener('click', function(e){
    e.preventDefault();
    const libform=document.querySelector('#libform');
    const noun=document.querySelector('#noun');
    const adjective=document.querySelector('#adjective');
    const person=document.querySelector('#person');
    const verb=document.querySelector('#verb');
    const place=document.querySelector('#place');
    if (!noun.value || !adjective.value || !person.value || !verb.value || !place.value) {
        alert('Please fill in all fields');
        return;
        }
    
    const story = `${person.value} went to ${place.value} and saw a ${adjective.value} ${noun.value}. They decided to ${verb.value} it!`;
    storySpan.innerText=story;
    
})

//    Make sure the values are not empty
//    Write a story that uses each of the values.
//    Make sure you check the console for errors when playing the game.

//    Bonus: Add a “shuffle” button to the HTML file, when clicked the
//       button should change the story currently displayed (but keep the 
//          values entered by the user). The user could click the button at least 
//           three times and get a new story. Display the stories randomly.
